...
Test and Roll (tactical A/B testing) describes an experimentation process in marketing where a subset of customers are randomly assigned to a treatment and customer response data is first collected in a "test" stage. In the "roll" stage that follows, marketers deploy one treatment to all remaining customers based on the test results.<br/>  
...
Profit-maximizing Test and Roll has several advantages over traditional hypothesis testing (see paper for more).  
...
test_size_nn() - computes profit-maximizing test sizes under test and roll   
...
profit_nn_sim() -  computes the per-customer profit for test and roll with K arms  
...
**profit_nn() computes the per-customer profit for test and roll with 2 arms**
```{r}
...
**test_size_nn_sim() Computes the profit-maximizing test size and profits for a multi-armed test and roll**
```{r, warning=FALSE}
...
```
Takes in parameters:  
"n" - sample sizes for test and roll (vector of length 1 or K)  
"N" - deployment population  
...
"K" - number of arms (2 by default)  
"TS" - whether or not to run Thomson sampling, 'TRUE' or 'FALSE' (FALSE by default)  
*and returns the profits and error rates under perfect information, test and roll, and Thomson sampling under one simulation*
<br/>
